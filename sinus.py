...
Udc = dc_component(y)
Uef = efective(y)
σ = sp.ndimage.standard_deviation(y[:N_uzoraka])
γ = (σ/Udc) * 100
Psr = Uef**2
Psr_dBW = 20 * np.log10(Uef) 
rounded_values = [round(x,16) for x in [Umax,Umin,Udc,Uef,σ,γ,Psr,Psr_dBW ]]

table = pd.DataFrame( {
                          "Values":rounded_values,
                          #"":["V","V","V","V","V","%"]
                          },
                        index= ["Umax","Umin","Udc","Uef","σ","γ","Psr","Psr_dBW "])
...
on = st.checkbox("Napravi analizu signala")
